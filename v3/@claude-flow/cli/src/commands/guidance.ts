...
const abTestCommand: Command = {
  ...
  options: [
    ...
    { name: 'json', type: 'boolean', description: 'Output as JSON', default: 'false' },
    { name: 'skip-warning', type: 'boolean', description: 'Skip default executor warning (not recommended - may produce zero-delta results)', default: 'false' },
  ],
  examples: [
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    const jsonOutput = ctx.flags.json === true;
    const skipWarning = ctx.flags['skip-warning'] === true;
    ...
    try {
      ...
      const report = await abBenchmark(configBContent, {
        tasks: customTasks,
        workDir,
        skipDefaultExecutorWarning: skipWarning,
      });
