...
describe('abBenchmark', () => {
  describe('executor content-awareness validation', () => {
    ...
    it('accepts content-aware executor', async () => {
      const executor = new ABDifferentialExecutor();
    ...
    it('accepts default executor (which is content-aware)', async () => {
      // Default executor should be content-aware and pass validation
      const report = await abBenchmark(WELL_STRUCTURED_CLAUDE_MD);
      expect(report).toBeDefined();
      expect(report.configA).toBeDefined();
      expect(report.configB).toBeDefined();
