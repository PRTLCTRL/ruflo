...
describe('abBenchmark', () => {
  ...
  describe('executor validation', () => {
    ...
    it('error message contains helpful guidance and issue link', async () => {
      const nonContentAwareExecutor = new CompliantExecutor();
      ...
      try {
        ...
        expect(error.message).toContain('github.com/ruvnet/ruflo/issues/1652');
      }
    });

    it('accepts content-aware executors without error', async () => {
      const contentAwareExecutor = new ABDifferentialExecutor();
      
      // Should not throw
      await expect(abBenchmark(WELL_STRUCTURED_CLAUDE_MD, {
