...
const handlers = {
  ...
  'pre-bash': () => {
    ...
    for (var i = 0; i < dangerous.length; i++) {
      if (cmd.includes(dangerous[i])) {
        console.error(JSON.stringify({ error: 'Dangerous command detected: ' + dangerous[i] }));
        process.exit(1);
      }
    }
  ...
  'pre-task': () => {
    ...
    if (router && router.routeTask && prompt) {
      ...
      console.log(JSON.stringify({ status: 'ok', message: 'Task routed to: ' + result.agent, confidence: result.confidence }));
    } else {
