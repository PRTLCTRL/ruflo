...
const handlers = {
  'route': () => {
    if (intelligence && intelligence.getContext) {
      try {
        ...
        if (ctx) console.log(JSON.stringify({ status: 'ok', context: ctx }));
      } catch (e) { /* non-fatal */ }
    }
  ...
  'pre-bash': () => {
    ...
    for (var i = 0; i < dangerous.length; i++) {
      if (cmd.includes(dangerous[i])) {
        console.error(JSON.stringify({ status: 'blocked', reason: 'Dangerous command detected: ' + dangerous[i] }));
        process.exit(1);
      }
    }
  ...
  'session-restore': async () => {
    ...
    console.log(JSON.stringify({ status: 'ok', message: 'Session restored', sessionId: sessionId }));
  },
  ...
  'session-end': async () => {
    ...
    if (intelligence && intelligence.consolidate) {
      ...
      if (consResult && consResult.entries > 0) {
        console.log(JSON.stringify({
          status: 'ok',
          message: 'Intelligence consolidated',
    ...
    console.log(JSON.stringify({ status: 'ok', message: 'Session ended' }));
  },
  ...
  'stats': () => {
    if (intelligence && intelligence.stats) {
      ...
      console.log(JSON.stringify({ 
        status: 'warn',
        message: 'Intelligence module not available. Run session-restore first.'
      }));
    }
  },
};
...
if (command && handlers[command]) {
    try {
      ...
      console.log(JSON.stringify({ 
        status: 'warn',
        message: 'Hook ' + command + ' encountered an error',
        error: e.message
      }));
    }
  } else if (command) {
    ...
    console.log(JSON.stringify({ 
      status: 'error',
      message: 'Usage: hook-handler.cjs <route|pre-bash|post-edit|session-restore|session-end|pre-task|post-task|compact-manual|compact-auto|status|stats>'
    }));
  }
}
